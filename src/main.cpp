...
// Copyright (c) 2013 Omegacoin Developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Omegacoin: starting difficulty is 1 / 2^12
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if( nHeight < 2247816)
    {
    	nSubsidy = 10000 * COIN;
    }
    else 
    {
    	nSubsidy = 1000 * COIN;  // 1000 coins in perpetuity
    }
...
static const int64 nTargetTimespan =  0.10 * 24 * 60 * 60; // 10 times per day
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // Omegacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe66ef85a3f0b5942ab909f34b0ee1c2e67242a1fad4fd446c7ea3983f07909e1");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Alpha and Omega - 1/30";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402359525;  //epoch time
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 166273;

        if (fTestNet)
        {
            block.nTime    = 1402359525;
            block.nNonce   = 166273;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xf877ee6b268539f3ded9306408674ec9947550a80248a152a5dfddeefe1327be"));
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Omegacoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
